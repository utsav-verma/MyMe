...
const UnifiedChat: React.FC = () => {
  ...
  const handleMessageClick = useCallback((message: any) => {
    if (!message.fromMe && message.from) {
      ...
      const contact = {
        id: contactId,
        name: existingContact?.name || getContactName(contactId),
  ...
  return (
    <div className={`flex flex-col h-screen ${themeClasses.bg} ${themeClasses.text}`}>
      ...
      {/* Header */}
      <div className={`${themeClasses.headerBg} text-white p-4 flex items-center justify-between border-b ${themeClasses.border}`} style={{ paddingTop: 'calc(env(safe-area-inset-top) + 44px)' }}>
                  ...
                  <div className={`flex ${message.fromMe ? 'justify-end' : 'justify-start'} mb-1`}>
                    ...
                    >
                      ...
                      <p className="text-sm leading-relaxed whitespace-pre-wrap">
                        {(() => {
                          if (!message.body || message.body.trim() === '') return 'Media message';
                          
                          // Check if message is just a URL
                          if (message.body.startsWith('https://pps.whatsapp.net/') || 
                              message.body.match(/^https:\/\/[^\s]*$/)) {
                            return 'Media message';
                          }
                          
                          // Remove all WhatsApp URLs and clean up
                          let cleanBody = message.body
                            .replace(/https:\/\/pps\.whatsapp\.net\/[^\s\n]+/g, '')
                            .replace(/https:\/\/[^\s\n]*whatsapp[^\s\n]*/gi, '')
                            .replace(/\n+/g, '\n')
                            .trim();
                          
                          return cleanBody || 'Media message';
                        })()
                        }
                      </p>
          ...
          <div className={`${themeClasses.modalBg} rounded-lg p-6 w-full max-w-md`}>
            <div className="flex items-center mb-4">
              <div className="flex items-center gap-3 flex-1">
                {chatState.selectedContactForReply.profilePicUrl ? (
                  <img 
                    src={chatState.selectedContactForReply.profilePicUrl} 
                  ...
                  <div className={`w-12 h-12 rounded-full ${isDarkMode ? 'bg-gray-600' : 'bg-gray-300'} flex items-center justify-center`}>
                    <span className="text-xl">{getContactAvatar(chatState.selectedContactForReply.id)}</span>
                  </div>
                )}
                <div>
                  <h3 className={`font-semibold ${themeClasses.text} text-lg`}>
                    {getContactName(chatState.selectedContactForReply.id)}
                  </h3>
                  <p className={`text-sm ${themeClasses.textSecondary}`}>Active now</p>
                </div>
              </div>
              <button
